...
settings <- list(
  ...
  ### final model output
  dir_out = "/storage/research/giub_geco/data_2/scratch/akurth/grsofun_output/remap",
  dir_out_nc = "/storage/research/giub_geco/data_2/scratch/akurth/grsofun_output/remap",
  save = list(gpp = "mon"),
  overwrite = FALSE,
...
tictoc::tic("Tidy input processing")
...
# -----------------------------------------------------------
# Check which files were not created, run for missing LON
# -----------------------------------------------------------
df_lon_index <- map2tidy::get_df_lon_index(settings$grid)
expected <- map2tidy::get_file_suffix(df_lon_index$lon_index, df_lon_index)
existing <- list.files(
  settings$dir_out,
  pattern = "\\.rds$", full.names = FALSE
)
created <- gsub(paste0("^", settings$fileprefix, "|\\.rds$"), "", existing)
missing <- setdiff(expected, created)
length(missing)
head(missing)
results <- purrr::map(missing, ~ grsofun_run_byLON(.x, par, settings))

missing <- purrr::keep(expected, ~ !file.exists(
  file.path(settings$dir_out, paste0(settings$fileprefix, .x, ".rds"))
))
print(missing)
